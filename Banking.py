...
transactionCommands = ["help", "cancel"]
...
class Account:
  ...
  def makeLoanDeposit(self, idx, deposit):
    ...
    self.balance = float(self.balance - deposit)
...
class Loan:
  ...
  def makeDeposit(self, deposit):
    self.amount -= float(deposit)

  def addMonth(self):
    self.time += 1
    self.amount = float(self.amount * currentLoanInterest)


clientsInfo.append(Account("siper", 4000.00))

clientsInfo[0].loans.append(Loan(200.00))
...
def login():
  ...
  while not succeeded:
    ...
    if command == "help":
      ...
      print("Username/command does not exist, please try again or type \"help\" for possible commands.\n")
  loggedIn()
...
def loggedIn():
  ...
  while online:
    ...
    if command == "help":
      ...
      print("Command does not exist, please try again or type \"help\" for possible commands.\n")
...
def newLoan():
  ...
  if len(clientsInfo[userIdx].loans) == maxLoans:
    print("You can not get a new loan. You already have " + str(maxLoans) + " loans.")
  else:
    while not completed:
      ...
      if command == "help":
        ...
        if correctMoney(command):
          if float(command) < maxLoanAmount:
            ...
            print("Your new loan has been created with an amount of " + str(float(command)) + " euros\n")
            completed = True
          else:
            print("The max amount of a loan is " + str(maxLoanAmount) + " euros. Please input a number below this amount.")
...
def makeDeposit():
  ...
  while not completed:
    command = input("How much money would you like to put into your account?\n")
    if command == "help":
      possibleCommands(transactionCommands)
    elif command == "cancel":
      completed = True
      ...
      if correctMoney(command):
        ...
        print("You have successfully deposited " + str(float(command)) + " euros\n")
...
def makeLoanDeposit():
  ...
  while not completed:
    command = input("What loan would you like to make a deposit on? (look at the number before the loans, example: 1)\n")
    if command == "help":
      possibleCommands(transactionCommands)
    elif command == "cancel":
      completed = True
      ...
      try:
        ...
        if index <= len(clientsInfo[userIdx].loans) and index > 0:
          ...
          while not completed2:
            command = input("How much money would you like to deposit from your account to the selected loan?\n")
            if command == "help":
              possibleCommands(transactionCommands)
            elif command == "cancel":
              completed2 = True
              ...
              if correctMoney(command):
                if float(command) <= clientsInfo[userIdx].balance:
                  if float(command) <= clientsInfo[userIdx].loans[loanIdx].amount:
                    clientsInfo[userIdx].makeLoanDeposit(loanIdx, float(command))
                    print("You have successfully deposited " + str(float(command)) + " euro to loan number " + str(index) + "\n")
                    completed2 = True
                  else:
                    print("You can't deposit more money than needed to pay off your loan.\n")
                else:
                  print("You don't have enough money in your balance to pay this amount.\n")
        else:
          print("This loan does not exist, please try again.\n")
      except ValueError:
        print("Please provide a full number like 1, 2 or 3 to select your loan.\n")
...
def makeWithdrawal():
  ...
  while not completed:
    command = input("How much money would you like to retrieve from your account?\n")
    if command == "help":
      possibleCommands(transactionCommands)
    elif command == "cancel":
      completed = True
      ...
      if correctMoney(command):
        if (clientsInfo[userIdx].balance >= float(command)):
          ...
          print("You have successfully withdrawn " + str(float(command)) + " euros\n")
          totalMoney -= float(command)
          completed = True
        else:
          print("You do not have the balance to withdraw this amount of money, please input a different amount or type \"cancel\"")
...
def showAccount():
  ...
  for idx, loan in enumerate(client.loans):
    print(str(idx + 1) + "\t" + str(loan.amount) + " euro\t" + str(currentLoanInterest*100) + "%\t\t\t" + str(loan.amount + (loan.amount * currentLoanInterest)) + " euro\t\t" + str(loanMaxMonths - loan.time) + " month(s)")
  print("")
    ...
    try:
      ...
      if len(values) == 2:
        ...
        if len(values[1]) <= 2:
          ...
          print("Please enter a valid amount consisting of no more than 2 numbers after the \".\"\nPlease try again\n")
          return False
      else:
